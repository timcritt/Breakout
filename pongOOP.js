...
const brick = {
    row: 3,
    column: columns,
    width: (canvas.width -(offsetLeft*columns)-offsetLeft)/columns, 
    height: 20,
    ...
    for (var r = 0; r < brick.row; r++) {
        ...
        for (var c = 0; c < brick.column; c++) {
            ...
            if (r%2 == 0) {
                ...
                indent = (brick.width + brick.offSetLeft)/2;
            }

            //indented rows have one less brick than columns
            if (indent != 0 && c == brick.column-1) {
                bricks[r][c] = {
                    x: c * (brick.offSetLeft + brick.width) + brick.offSetLeft + indent,
